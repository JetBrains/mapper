...
public class PropertySpec<ValueT> {
  ...
  public static <ValueT> PropertySpec<ValueT> of(String name, final ValueT defaultValue) {
    return new PropertySpec<>(name, new Function<WithProperties, ValueT>() {
      @Override
      public ValueT apply(WithProperties model) {
        return defaultValue;
      }
    });
  }

  public static <ValueT> PropertySpec<ValueT> of(String name, Function<WithProperties, ValueT> defaultValue) {
    return new PropertySpec<>(name, defaultValue);
  }

  private final String myName;
  private final Function<WithProperties, ValueT> myDefaultValue;

  protected PropertySpec(String name, Function<WithProperties, ValueT> defaultValue) {
    if (name == null) {
  ...
  public ValueT getDefaultValue(WithProperties model) {
